...
class HistoryWrapper(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        ...
        if (self.last_obs is None):
            ...
            #ajout de l'obsevration a la fin
            frames = {
                "discrete": [observation["discrete"]] + [self.last_obs["discrete"]],
                "continuous": [observation["continuous"]] + [self.last_obs["continuous"]]
            }
        self.last_ob = observation#save the obs for the next run
        return {
            "discrete": torch.tensor(np.concatenate(frames["discrete"]), dtype=torch.long),
            "continuous": torch.tensor(np.concatenate(frames["continuous"]), dtype=torch.float32)
        }
...
#------------------ RECCURENT PPO -------------------------#
# For PPO it is only a full discrete world
class LstmAgent():
    def __init__(self, state_dim, input_lstm_size, hidden_size, num_envs, device):

        self.features_extractor = nn.Linear(state_dim, input_lstm_size).to(device)
        self.lstm = nn.LSTM(input_lstm_size, hidden_size).to(device)
        self.device = device
    ...
    def init_lstm_state(self, batch=-1):
      if (batch == -1):
        batch = self.num_envs

      self.lstm_state = (
            torch.zeros(self.lstm.num_layers, batch, self.lstm.hidden_size).to(self.device),
            torch.zeros(self.lstm.num_layers, batch, self.lstm.hidden_size).to(self.device),
      )

    def forward(self, t, observation, done, **kwargs):

        force_update = False
        if ("is_running" in kwargs and kwargs["is_running"] == True):
          force_update = True
          self.init_lstm_state(1)

        lstm_state_forward = self.lstm_state

        if ("lstm_state" in kwargs):
          self.init_lstm_state()
          lstm_state_forward = kwargs["lstm_state"]

        batch_size = lstm_state_forward[0].shape[1]

        hidden =  self.features_extractor(observation)
        ...
        for h, d in zip(hidden, done):
            ...
            h, lstm_state_forward = self.lstm(
                h.unsqueeze(0),
                (
                    (1.0 - d.float()).view(1, -1, 1) * lstm_state_forward[0],
                    (1.0 - d.float()).view(1, -1, 1) * lstm_state_forward[1],
                ),
            )
            new_hidden += [h]
        ...
        if ("update_lstm_state" in kwargs and kwargs["update_lstm_state"] == True) or force_update:
          self.lstm_state = (lstm_state_forward[0].detach(), lstm_state_forward[1].detach())

        return new_hidden

class PPOPolicy(Agent):
    def __init__(self, input_size, hidden_size, n_actions, head, name="policy"):
        super().__init__(name=name)
        self.head = head
        self.model = build_ortho_mlp(
            [input_size] + list(hidden_size) + [n_actions], activation=nn.ReLU()
        )
        for layer in self.model:
          if isinstance(layer, nn.Linear):
            ortho_init(layer, std=0.01)
